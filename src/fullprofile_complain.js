...
function Giveid(){
  let  param=useParams();
return < FullProfile_Missing id={param.id}/>
}
...
class FullProfile_Missing extends React.Component {
  ...
  const response = await fetch("http://127.0.0.1:8000/mycomplain/matchinfoundedface",{
  method: "POST",
  headers: {
       'Content-Type': 'application/json',
       'Authorization': `bearer ${token}`
...
<h2>Some People  We Found Regarding Your Missing Complain</h2>
